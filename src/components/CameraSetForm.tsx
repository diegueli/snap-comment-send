...
import { X, Camera as CameraIcon, RotateCcw, Save, ArrowLeft } from 'lucide-react';
...
const CameraSetForm: React.FC<CameraSetFormProps> = ({ onClose, onBack, userData, auditoriaData }) => {
  ...
  const onSubmit = async (data: AuditSetFormData) => {
    ...
    try {
      ...
      // Reset form and image for next set
      form.reset();
      setImage(null);
    } catch (error: any) {
      ...
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle>Set de Auditoría - {auditoriaData.tituloDocumento}</CardTitle>
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">
            <p><strong>Planta:</strong> {auditoriaData.plantaNombre}</p>
            <p><strong>Auditor:</strong> {auditoriaData.auditor}</p>
            <p><strong>Fecha:</strong> {auditoriaData.fecha}</p>
          </div>
